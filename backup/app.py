...
def convert_time_to_datetime(time_str):
    ...
    # Handle "days ago"
        ...
        try:
            # Remove ordinal suffix (e.g., "25th" -> "25")
...
def group_sentences(word_list):
    ...
    for word in word_list:
        if word[:-1].isdigit() and word.endswith('.'):  # Check if it's a number followed by '.'
            if temp_sentence:
                result.append(" ".join(temp_sentence))  # Append previous sentence before starting a new one
            temp_sentence = [word]  # Start a new sentence
        else:
...
def send_email(sender, receiver, body, pwd= email_pwd):
    """Sends automatic email notification to the developer when a user requests the service.

    (Data Processing Policy: Only includes user device information—hostname, operating system, timestamp, and API usage data.)
...
def send_bug_report(sender, receiver, body, pwd= email_pwd):
    """Sends automatic email notification to the developer when a user encounters a bug.

    (Data Processing Policy: Only includes the traceback bug report.)
    ...
    try:
        ...
        smtp_server.sendmail(sender, receiver, m.as_string())
        smtp_server.quit()
        st.write('We received the notification and will be working on it.')
    except Exception as e:
        st.error("Failed to send bug report. Please contact support.")
...
set_bg_hack('graphics/wwdc-glowing-violet-3840x2160-19118.png')
#st.cache_data.clear()
...
try:
    ...
    lottie_animation2 = get("graphics/Scene-1.json")
    ...
    if custom_selected_topic and selected_topic== None:
        lottie_animation1 = get("graphics/Animation_9.json")
        ht= 450
    else:
        lottie_animation1 = get("graphics/Animation_7.json")
        ht= 300
    ...
    if prompt:
        try:
            ...
            if response.text and response.text.strip():  # Check if response is non-empty
                ...
                answer = group_sentences(words_list)
            else:
    ...
    try:
        ...
        if email_notification:
            send_email("glimpsethrough98@gmail.com", "glimpsethrough98@gmail.com", body, pwd= email_pwd)
    except:
        pass
    ...
    if isresponse and extract_response:
        ...
        for medium in extract_response:
            df= ScrapData(topic= topic, news_medium=medium, domain="https://news.google.com/", count=news_count, region=region_code, isenglish=isenglish, isdefault= isdefault) 
    ...
    try:
        ...
        """
        if email_notification:
            send_bug_report("glimpsethrough98@gmail.com", "telang.sarvesh98@gmail.com", body, pwd= email_pwd)
    except:
        pass

    lottie_animation3 = get("graphics/Animation_10.json")
